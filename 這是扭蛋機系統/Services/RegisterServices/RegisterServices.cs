    ...
    public class RegisterService : IRegisterServices
    {
        ...
        public async Task<Register> LoginAsync(string account, string password)
        {
            try
            {
                ...
                Console.WriteLine($"登入失敗: {errorResponse}");

                return null; // 登入失敗時回傳 `null`
            }
            catch (HttpRequestException httpEx)
            {
                Console.WriteLine($"網路請求錯誤: {httpEx.Message}");
                return null;
            }
            catch (Exception ex)
            {
        ...
        {
            string url = $"{BaseUrl}/search?keyword={keyword}&searchType={searchType}";
            ...
            {
                var json = await response.Content.ReadAsStringAsync();
                return JsonSerializer.Deserialize<IEnumerable<Member>>(json, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
            }
