...
namespace 這是扭蛋機系統.Services.SubProductsService
{
    internal class SubProductsService : ISubProductsService
    {
        ...
        // ✅ **新增小商品**
        public async Task<bool> AddSubProductAsync(int productId, string subProductName, string imagePath, int stock)
        {
            var request = new
            {
                P_ID = productId,
                SP_Name = subProductName,
                SP_Image = Path.GetFileName(imagePath), // ✅ 只存圖片名稱
                SP_Amount = stock
            };
        ...
        public async Task<List<UpdateSubProducts>> UpdateGetAllSubProductsAsync()
        {
            var response = await _httpClient.GetAsync($"{BaseUrl}/by-product");
            if (response.IsSuccessStatusCode)
            {
                var json = await response.Content.ReadAsStringAsync();
                return JsonSerializer.Deserialize<List<UpdateSubProducts>>(json, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
            }
            return new List<UpdateSubProducts>();
        }
