...
namespace 這是扭蛋機系統
{
    public partial class FormPractical : Form
    {
        ...
        private List<Products> practicalProducts = new List<Products>();
        private int _userId;
        public FormPractical(int userId)
        {
            InitializeComponent();
            _productsService = new ProductsService();
            this.Load += async (s, e) => await LoadPracticalProductsAsync();
            this._userId = userId; // ✅ 設定 UserID
        }

        private async Task LoadPracticalProductsAsync()
        {
            var products = await _productsService.GetAllProductsAsync();
            practicalProducts = products.Where(p => p.P_Class == "其他類").ToList();
            ShowProductsForPage(currentPage);
        }

        private void ShowProductsForPage(int page)
        {
            flowLayoutPanelPracticalProducts.Controls.Clear();

            int startIndex = (page - 1) * itemsPerPage;
            int endIndex = Math.Min(startIndex + itemsPerPage, practicalProducts.Count);

            for (int i = startIndex; i < endIndex; i++)
            {
                var product = practicalProducts[i];
                ...
                flowLayoutPanelPracticalProducts.Controls.Add(productPanel);
            }
        ...
        {
            int totalPages = (int)Math.Ceiling((double)practicalProducts.Count / itemsPerPage);
