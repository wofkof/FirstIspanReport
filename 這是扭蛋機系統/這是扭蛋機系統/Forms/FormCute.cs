...
namespace 這是扭蛋機系統
{
    public partial class FormCute : Form
    {
        private readonly ProductsService _productsService;
        private int currentPage = 1;
        private int itemsPerPage = 8;
        private List<Products> cuteProducts = new List<Products>();
        private int _userId;
        public FormCute(int userId)
        {
            InitializeComponent();
            _productsService = new ProductsService();
            this.Load += async (s, e) => await LoadCuteProductsAsync();
            this._userId = userId; // ✅ 設定 UserID
        }

        private async Task LoadCuteProductsAsync()
        {
            var products = await _productsService.GetAllProductsAsync();
            cuteProducts = products.Where(p => p.P_Class == "可愛類").ToList();
            ShowProductsForPage(currentPage);
        }

        private void ShowProductsForPage(int page)
        {
            flowLayoutPanelCuteProducts.Controls.Clear();

            int startIndex = (page - 1) * itemsPerPage;
            int endIndex = Math.Min(startIndex + itemsPerPage, cuteProducts.Count);

            for (int i = startIndex; i < endIndex; i++)
            {
                var product = cuteProducts[i];
                ...
                flowLayoutPanelCuteProducts.Controls.Add(productPanel);
            }
        ...
        {
            int totalPages = (int)Math.Ceiling((double)cuteProducts.Count / itemsPerPage);
