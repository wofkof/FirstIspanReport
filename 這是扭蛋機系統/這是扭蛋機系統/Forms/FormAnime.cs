...
namespace 這是扭蛋機系統
{
    public partial class FormAnime : Form
    {
        ...
        public FormAnime(int userId)
        {
            InitializeComponent();
            _productsService = new ProductsService();
            this.Load += async (s, e) => await LoadAnimeProductsAsync();
            this._userId = userId; // ✅ 設定 UserID
        }

        private async Task LoadAnimeProductsAsync()
        {
            var products = await _productsService.GetAllProductsAsync();
            animeProducts = products.Where(p => p.P_Class == "動漫類").ToList();
