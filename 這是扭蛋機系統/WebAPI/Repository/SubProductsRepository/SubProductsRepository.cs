...
namespace WebAPI.Repository.NewFolder1
{
    public class SubProductsRepository : ISubProductsRepository
    {
            ...
            // 新增小商品
            public async Task<int> AddSubProductAsync(SubProducts subProduct)
        {
            var sql = @"INSERT INTO SubProducts (P_ID, SP_Name, SP_Amount, SP_Image) 
                        VALUES (@P_ID, @SP_Name, @SP_Amount, @SP_Image);
                        SELECT CAST(SCOPE_IDENTITY() AS INT);";
            using (var connection = _dbService.GetDbConnection())
            {
                return await connection.ExecuteScalarAsync<int>(sql, subProduct);
            }
        }
        // 刪除小商品
