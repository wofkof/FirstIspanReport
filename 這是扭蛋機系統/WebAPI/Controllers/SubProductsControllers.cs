...
namespace WebAPI.Controllers
{
    ...
    public class SubProductsController : ControllerBase
    {
        ...
        {
            if (subProduct == null)
                return BadRequest("小商品資訊不可為空");

            int newSubProductId = await _subProductsRepository.AddSubProductAsync(subProduct);
            return CreatedAtAction(nameof(GetSubProductById), new { spId = newSubProductId }, subProduct);
        }
