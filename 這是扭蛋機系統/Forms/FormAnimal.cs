...
namespace 這是扭蛋機系統
{
    public partial class FormAnimal : Form
    {
        ...
        private List<Products> animalProducts = new List<Products>();
        private int _userId; // ✅ 存儲登入的 UserID

        public FormAnimal(int userId)
        {
            InitializeComponent();
            _productsService = new ProductsService();
            this.Load += async (s, e) => await LoadAnimalProductsAsync();
            this._userId = userId; // ✅ 設定 UserID
        }

        private async Task LoadAnimalProductsAsync()
        {
            var products = await _productsService.GetAllProductsAsync();
            animalProducts = products.Where(p => p.P_Class == "動物類").ToList();
            ShowProductsForPage(currentPage);
        }

        private void ShowProductsForPage(int page)
        {
            flowLayoutPanelAnimalProducts.Controls.Clear();

            int startIndex = (page - 1) * itemsPerPage;
            int endIndex = Math.Min(startIndex + itemsPerPage, animalProducts.Count);

            for (int i = startIndex; i < endIndex; i++)
            {
                var product = animalProducts[i];
                ...
                flowLayoutPanelAnimalProducts.Controls.Add(productPanel);
            }
        ...
        {
            int totalPages = (int)Math.Ceiling((double)animalProducts.Count / itemsPerPage);
